...
import { createPool, closePool, query } from "../src/db/pool.js";
...
test("integration: 5 concurrent lockNextAvailable calls return 5 distinct group names", { skip: !hasEnv && "MYSQL_TEST_* not set" }, async () => {
  ...
  // Apply migration (idempotent CREATE TABLE IF NOT EXISTS). Strip comment
  // lines first so they don't get glued onto the first CREATE TABLE.
  ...
  const results = await Promise.all(
    Array.from({ length: 5 }, (_, i) =>
      groupsRepo.lockNextAvailable(accountId, String(1000 + i), 24),
    ),
  );
  ...
  const sixth = await groupsRepo.lockNextAvailable(accountId, "9999", 24);
